...
\begin{figure}
\caption{A single neuron firing rate test family implemented in \textit{SciUnit}}
\label{fig:rate_test}
\begin{python}
class SpikeCountTest(sciunit.Test):
  ...
  required_capabilities = (
    ...
    neurounit.capabilities.ProducesSpikeCount
  )
	
  def run_test(self, model):
    for (input, mean, std) in zip(self.inputs, self.means, self.stds):
      model.set_current(input)
