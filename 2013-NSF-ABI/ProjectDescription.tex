...
% Josh says: A lot of the *potential* and actual features of SciDash are
%scattered throughout the document. This gives the reader the feeling
%that its features aren't fully thought out. 

% Sharon says: The only more substantial comment has to do with the workflow. You write about the model description and then say will simulate via neuroconstruct, then apply tests comparing simulated data with experimental data. All that is clear.  What is not quite so clear to me is the specification of the simulation details since they must match the experimental methods. In other words, if I were reviewing I would say that one weakness is the lack of details about how the details of the simulations are determined or perhaps where this information is stored. How is this automated? If you are running a suite of tests on a model, does triggering the suite trigger a certain simulation protocol to get the needed computational outpub? This part is not clear to me.

% Jonathan says: Regarding model families--I think we should use "model" for the class and "model instance" for the instance.  In other words, to me the English word "model" is a class-like thing that is parameterized and can be instantiated.  For example, we speak of a "linear model" of a neuron - in fact you even call the class a LinearModel, not a LinearModelFamily.

% Jonathan says: All I meant was give me an overview of the plan (and more generally an outline for section 4) before going into the weeds.  It's somewhat uncomfortable to read section 4.1 with no idea whatsoever of what will be coming in 4.2.  Maybe that's the CS dude in me who wants every bit in its place, but I think foreshadowing things is helpful.

% Jonathan says: It might make sense to say a bit more in the coordination section.  In particular, we have funds to travel (e.g. for Cyrus to visit ASU) and mentioning an approach to meetings that we plan to take might be a good idea also.

% Josh says: In particular I'd like to
...
% Jonathan said: I think this is a good thing for us to do.  We could%
